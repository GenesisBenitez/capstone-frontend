    ...
    return(
        <div className="row justify-content-center">
            <div className="col-10 col-sm-10 col-md-10 col-lg-10 col-xl-10">
                ...
                <h1 className='lead mt-4'>{countryBooks.length > 0 ? `Books from ${country.name}` : ""} </h1>
                <div className="row justify-content-center">
                    <div className="col-10 col-sm-10 col-md-10 col-lg-10 col-xl-10">
                        <Carousel className='mt-4' responsive={responsive}>
                            {countryBooks.map((countryBook, i)=>(
                                <Link to={`/book/${countryBook.id}`}><img src={countryBook.cover} height="250"/></Link>
                            ))}
                            
                        </Carousel>
                        </div>
                </div>
                <h1 className='lead mt-4'>{countryFilms.length > 0 ? `Films from ${country.name}` : ""} </h1>
